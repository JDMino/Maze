...
function drawPlayer() {
  ...
  // Shield ring
  if(shieldActive) {
    const alpha = GFX.shieldPulse ? 0.6+Math.sin(t*8)*0.2 : 0.7;
    ctx.globalAlpha=alpha;
    ctx.strokeStyle='#00f5ff';
    ctx.lineWidth=2;
    if(GFX.entityGlow) { ctx.shadowColor='#00f5ff'; ctx.shadowBlur=15; }
    ctx.beginPath();
...
// ─── INIT ────────────────────────────────────
updateMenuStats();
// Sync quality selector buttons to saved preference
document.querySelectorAll('.quality-btn').forEach(b => {
